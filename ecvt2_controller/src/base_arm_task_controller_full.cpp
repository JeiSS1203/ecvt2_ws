...
public:
    ...
    EcvtController() : Node("base_arm_task_controller_full")
    {
        urdf_path_ = this->declare_parameter<std::string>(
            "urdf_path",
            ament_index_cpp::get_package_share_directory("forestry_robot_mjcf") + "/xml/ecvt_v2.urdf");
        RCLCPP_INFO(this->get_logger(), "Using Pinocchio URDF: %s", urdf_path_.c_str());
        ...
        RCLCPP_INFO(this->get_logger(), "EcvtController full-body initialized with 100Hz loop");
...
private:
    ...
    void controlLoop()
    {
        ...
        const auto fk_result = ecvt2_controller::computePinocchioFKAllAndJacobian(
            base_pos_, base_quat_normalized, q_, urdf_path_);
