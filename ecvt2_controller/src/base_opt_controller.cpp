...
private:
    ...
    void controlLoop()
    {
        ...
        Eigen::Matrix3d A = computeManipulabilityMatrix(result, q_);
        Eigen::Vector3d grad = computeVMaxDirection(base_pos_, base_quat_, q_, A);
        double grad_gain = grad.norm();
        if (grad_gain >= 0.2)
        {
            grad_gain = 0.2;
        }
        ...
        // ---------------------------------------------------------
        // 1. Contact Nullspace Projection Matrix (Pc) 계산
        // ---------------------------------------------------------
        // Contact Jacobian (다리 4개, 12 constraints: FR_pos ~ RL_pos)
        // J_full index 6~17
        Eigen::MatrixXd J_c = J_full.block<12, 26>(6, 0);

        // Pc = I - Jc_pinv * Jc
        // SVD를 이용한 Pseudo-Inverse 계산 (안전성 확보)
        Eigen::JacobiSVD<Eigen::MatrixXd> svd(J_c, Eigen::ComputeThinU | Eigen::ComputeThinV);
        double tolerance = 1e-4; // 특이값 임계치
        Eigen::MatrixXd J_c_pinv = svd.matrixV() * (svd.singularValues().array().abs() > tolerance).select(svd.singularValues().array().inverse(), 0).matrix().asDiagonal() * svd.matrixU().transpose();

        Eigen::MatrixXd I = Eigen::MatrixXd::Identity(26, 26);
        Eigen::MatrixXd P_c = I - (J_c_pinv * J_c); // [26 x 26]

        // ---------------------------------------------------------
        // 2. Task Setup (Stacking EE + Base Orientation)
        // ---------------------------------------------------------
        
        // (Task A) End-Effector Position (3 DoF)
        ...
        // (Task B) Base Orientation (3 DoF)
        Eigen::MatrixXd J_base_ori = J_full.block<3, 26>(3, 0); // Base Rotation 부분
        
        // Orientation Error (Rotation Matrix Column Cross Product)
        // R_current.col(i) cross R_desired.col(i)
        Eigen::Matrix3d R_cur = result.T[0].block<3, 3>(0, 0);
        Eigen::Vector3d base_ori_err = R_cur.col(0).cross(base_R_desired_.col(0)) +
                                       R_cur.col(1).cross(base_R_desired_.col(1)) +
                                       R_cur.col(2).cross(base_R_desired_.col(2));
        
        Eigen::Vector3d v_cmd_base_ori = 3.0 * base_ori_err; // Gain 3.0

        // (Task C) Base Manipulability Maximization (추가)
        Eigen::MatrixXd J_base_lin = J_full.block<3, 26>(0, 0); // Base Linear Velocity 부분
        Eigen::Vector3d v_cmd_base_lin = grad_gain * grad; // grad 방향


        // (Stacking) Combine Tasks into one matrix/vector
        Eigen::MatrixXd J_task_stacked(9, 26);
        Eigen::VectorXd v_task_stacked(9);

        J_task_stacked.block<3, 26>(0, 0) = J_ee;       // 상단 3행: EE
        J_task_stacked.block<3, 26>(3, 0) = J_base_ori; // 하단 3행: Base Ori
        J_task_stacked.block<3, 26>(6, 0) = J_base_lin; // 추가된 3행: Base Lin

        v_task_stacked.segment<3>(0) = v_cmd_ee;
        v_task_stacked.segment<3>(3) = v_cmd_base_ori;
        v_task_stacked.segment<3>(6) = v_cmd_base_lin;

        // ---------------------------------------------------------
        // 3. Jacobian Projection (핵심: 모든 Task에 Pc 적용)
        // ---------------------------------------------------------
        // 발이 고정된 Nullspace 위에서만 Task를 수행하도록 강제
        Eigen::MatrixXd J_final = J_task_stacked * P_c; 

        // ---------------------------------------------------------
        // 4. QP Formulation
        // ---------------------------------------------------------
        // Cost Function: || J_final * qdot - v_task_stacked ||^2

        Eigen::MatrixXd S = Eigen::MatrixXd::Zero(9,9);
        double s_ee   = 1.0;
        double s_ori  = 2.0;
        double s_lin  = 0.5;
        S.block<3,3>(0,0) = s_ee  * Eigen::Matrix3d::Identity();
        S.block<3,3>(3,3) = s_ori * Eigen::Matrix3d::Identity();
        S.block<3,3>(6,6) = s_lin * Eigen::Matrix3d::Identity();
        
        // Hessian (H) = J^T * J + Damping
        Eigen::MatrixXd H = J_final.transpose() * S * J_final;
        H.diagonal().array() += (lambda_vel + lambda_acc);

        // Gradient (g) = -J^T * v_cmd - Smoothing_Term
        Eigen::VectorXd g = -1.0 * J_final.transpose() * S * v_task_stacked;
        g -= lambda_acc * q_dot_prev_; // 이전 속도 유지하려는 관성항

        Eigen::SparseMatrix<double> H_sparse = H.sparseView();

        // ---------------------------------------------------------
        // 5. Constraints Setup (Joint Limits + Velocity Limits)
        // ---------------------------------------------------------
        
        // (A) 기본 설정
        double dt = 0.01;      
        double v_lim_base = 0.5; // Base 속도 한계
        
        // [Safety] 전체 관절 범위의 90%만 사용 (양쪽 5%씩 마진)
        double safety_ratio = 0.9; 

        // (B) 전체 관절 한계 벡터 설정 (Physical Limits)
        ...
        // --- User Defined Actuator Limits (Hardware Absolute Limits) ---
        // FR (6~9)
        ...
        // [Safety Scaling] 물리적 한계를 안전 한계(Safe Limit)로 축소
        // Actuator Index: 6 ~ 25
        for(int i=6; i<26; ++i)
        {
            double center = (q_min_full(i) + q_max_full(i)) / 2.0;
            double half_range = (q_max_full(i) - q_min_full(i)) / 2.0;
            
            // 범위를 90%로 축소
            double safe_half_range = half_range * safety_ratio;
            
            q_min_full(i) = center - safe_half_range;
            q_max_full(i) = center + safe_half_range;
        }
        ...
        for(int leg=0; leg<4; ++leg) {
            for(int j=0; j<4; ++j) q_map[6 + leg*4 + j] = hw_idx++;
            hw_idx++; 
        }
        for(int j=0; j<4; ++j) q_map[22 + j] = hw_idx++;

        for (int i = 0; i < 26; ++i)
        {
            // Case 1: Base (Floating Base) -> 고정 속도 제한
            if (i < 6) 
            {
                qp_lb(i) = -v_lim_base;
                qp_ub(i) =  v_lim_base;
            }
            // Case 2: Actuators (Legs + Arm) -> 안전 한계 적용
            else
            {
                int q_idx = q_map[i];
                if (q_idx < 0 || q_idx >= (int)q_.size()) {
                    qp_lb(i) = -v_lim_base; qp_ub(i) = v_lim_base; continue; 
                }

                double q_curr = q_(q_idx);
                // 이미 위에서 90%로 줄어든 q_min/max_full 사용
                double min_val = q_min_full(i);
                double max_val = q_max_full(i);

                // [Clamping Logic] 안전 한계(90%)를 벗어나면 0(Wall) 생성
                // 만약 91% 지점에 있다면? -> max_val(90%)로 Clamping됨 
                // -> UB = 0 (더 가는거 금지), LB = Open (복귀 허용)
                
                double raw_lb, raw_ub;
                raw_lb = (min_val - q_curr) / 1;
                raw_ub = (max_val - q_curr) / 1;
        ...
        if (iter_count_ % 50 == 0) 
        {
            std::cout << "\n--- [QP Bound Check (90% Safe) | Iter " << iter_count_ << "] ---" << std::endl;
            std::cout << "idx |  q_curr  | SafeMin  | SafeMax  ||   LB    |   UB" << std::endl;
            std::vector<int> debug_indices = {22, 23, 24, 25}; // Arm joints
            for (int i : debug_indices) {
                int q_idx = q_map[i];
                double q_c = (q_idx>=0 && q_idx<(int)q_.size()) ? q_(q_idx) : 0.0;
                printf(" %2d | %6.3f | %6.3f | %6.3f || %7.3f | %7.3f\n",
                       i, q_c, q_min_full(i), q_max_full(i), qp_lb(i), qp_ub(i));
            }
            std::cout << "----------------------------------------------------" << std::endl;
        }
        ...
        // A 행렬 (I * qdot) 설정
        int n_vars = 26;
        int n_constraints = 26; 
        Eigen::SparseMatrix<double> A_sparse(n_constraints, n_vars);
        A_sparse.setIdentity();

        // Solver 참조용 변수 연결
        Eigen::VectorXd &qp_lb_all = qp_lb;
        Eigen::VectorXd &qp_ub_all = qp_ub;
        ...
        if (!solver_local.data()->setLinearConstraintsMatrix(A_sparse))
            set_ok = false;
        if (!solver_local.data()->setLowerBound(qp_lb_all))
            set_ok = false;
        if (!solver_local.data()->setUpperBound(qp_ub_all))
            set_ok = false;

        Eigen::VectorXd q_dot_sol = Eigen::VectorXd::Zero(n_vars);
        if (set_ok && solver_local.initSolver())
        {
            if (solver_local.solveProblem() == OsqpEigen::ErrorExitFlag::NoError)
            {
                q_dot_sol = solver_local.getSolution();
                q_dot_prev_ = q_dot_sol;
                // [DEBUG] QP 해와 제약조건 비교 (Arm 위주)
                if (iter_count_ % 50 == 0)
                {
                    std::cout << "\n--- [QP Result Check (FR) | Iter " << iter_count_ << "] ---" << std::endl;
                    std::cout << "idx |   LB    | q_dot_sol |   UB    | Status" << std::endl;
                    
                    std::vector<int> debug_indices = {22, 23, 24, 25}; // Arm joints
                    for (int i : debug_indices) {
                        double lb = qp_lb(i);
                        double ub = qp_ub(i);
                        double sol = q_dot_sol(i);
                        
                        // 솔버 허용 오차(Tolerance) 감안해서 판별
                        bool safe = (sol >= lb - 1e-3) && (sol <= ub + 1e-3);
                        
                        printf(" %2d | %7.3f | %9.3f | %7.3f | %s\n",
                               i, lb, sol, ub, safe ? "OK" : "VIOLATION");
                    }
                    std::cout << "--------------------------------------------------------" << std::endl;
                }
            }
            ...
            {
                RCLCPP_WARN(this->get_logger(), "QP Solve Failed");
            }
        }
        // std::cout << "q_dot_sol: " << q_dot_sol.transpose() << std::endl;
        ...
        // BASE 속도를 내기위한 계산
        Eigen::VectorXd xdot_base = Eigen::VectorXd::Zero(6);
