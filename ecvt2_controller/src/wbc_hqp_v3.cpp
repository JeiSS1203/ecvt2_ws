...
private:
    ...
    void controlLoop()
    {
        ...
        // ---------------------------------------------------------
        // 11) Lower-body 3-stage HQP-like solve (22 vars)
        // - L1: wheel steering
        // - L2: base orientation
        // - L3: base linear
        //
        // Common hard constraints at every level:
        // - foot contact lock: J_contact * qdot = 0
        ...
        const Eigen::MatrixXd base_ori_jacobian_lower = full_jacobian.block<3, 22>(3, 0);
        const Eigen::MatrixXd base_lin_jacobian_lower = full_jacobian.block<3, 22>(0, 0);
        ...
        const Eigen::Vector3d base_ori_cmd = base_twist_world.segment<3>(3);
        const Eigen::Vector3d base_lin_cmd = base_twist_world.segment<3>(0);
        ...
        const double lambda_lower_qdot_smooth = 1e-3;
        Eigen::VectorXd qdot_lower_l3 = Eigen::VectorXd::Zero(kLowerDof);
        const bool is_lower_hqp_ok = hqp::solveLowerBodyHqp(
            wheel_jacobian, wheel_vel_cmd,
            base_ori_jacobian_lower, base_ori_cmd,
            base_lin_jacobian_lower, base_lin_cmd,
            contact_jacobian_lower,
            qdot_lower_limits, qdot_upper_limits,
            q_dot_prev_lower_,
            lambda_lower_qdot_norm, lambda_lower_qdot_smooth,
            qdot_lower_l3, qdot_lower_final);
        if (!is_lower_hqp_ok)
        {
            RCLCPP_WARN(this->get_logger(), "Lower-body HQP(3-level slack-lock) failed, fallback to previous lower-body command");
            qdot_lower_l3 = q_dot_prev_lower_;
            qdot_lower_final = q_dot_prev_lower_;
        }
        ...
        {
            RCLCPP_WARN(this->get_logger(), "Invalid/oversized lower HQP output detected. Falling back to previous lower-body command.");
            qdot_lower_final = q_dot_prev_lower_;
        }
        ...
        {
            const double wheel_residual_level1 = (wheel_jacobian * qdot_lower_final - wheel_vel_cmd).norm();
            const double base_ori_residual_level2 = (base_ori_jacobian_lower * qdot_lower_final - base_ori_cmd).norm();
            const double base_lin_residual_level3 = (base_lin_jacobian_lower * qdot_lower_final - base_lin_cmd).norm();
            const double contact_lock_residual = (contact_jacobian_lower * qdot_lower_final).norm();
            const double l3_to_final_residual = (qdot_lower_final - qdot_lower_l3).norm();
            ...
            if (wheel_residual_level1 > 5e-3 || base_ori_residual_level2 > 0.2 || base_lin_residual_level3 > 0.2 ||
                contact_lock_residual > 5e-4 || l3_to_final_residual > 1e-6 ||
                is_near_bound || !is_lower_hqp_ok)
            {
                RCLCPP_WARN(this->get_logger(),
                            "[diag-lower-v3-hqp3] ok=%d wheel_res=%.4e base_ori_res=%.4e base_lin_res=%.4e contact_lock_res=%.4e l3_to_final=%.4e max_abs_cmd=%.4f is_near_bound=%d min_margin_any=%.4e",
                            is_lower_hqp_ok ? 1 : 0,
                            wheel_residual_level1, base_ori_residual_level2, base_lin_residual_level3,
                            contact_lock_residual, l3_to_final_residual,
